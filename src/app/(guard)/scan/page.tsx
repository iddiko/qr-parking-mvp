...
export default function Page() {
      ...
      if (profileData?.complex_id) {
        ...
        if (profileData.building_id) {
          const { data: buildingData } = await supabaseClient
            ...
            .single();
          const buildingLabel = buildingData
            ? `${buildingData.code}동${buildingData.name ? ` ${buildingData.name}` : ""}`.trim()
            : "";
  ...
  return (
    <MenuGuard roleGroup="guard" toggleKey="scan">
      <div className="scan-page">
        <div className="scan-header">
          <h1 className="page-title">경비 QR 스캔</h1>
        ...
        {result ? (
          <div className="scan-card">
            <div className="scan-result">
              <span
                className={`scan-result-badge ${resultLabel === "입주민" ? "scan-result-badge--ok" : "scan-result-badge--warn"}`}
              >
                {resultLabel}
              </span>
